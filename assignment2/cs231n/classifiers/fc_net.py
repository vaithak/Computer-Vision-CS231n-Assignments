...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        X2, layer_1_cache = affine_relu_forward(X, W1, b1)
        scores, layer_2_cache = affine_forward(X2, W2, b2)
        ...
        grad_X2, grads['W2'], grads['b2'] = affine_backward(dx, layer_2_cache)
        _, grads['W1'], grads['b1'] = affine_relu_backward(grad_X2, layer_1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            ...
            if (self.normalization != None) and (i+1 != self.num_layers):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            X_next, curr_cache = affine_norm_relu_forward(X_curr ,W, b, gamma, beta, bn_params, self.normalization,                                              self.use_dropout, self.dropout_param)
            cache.append(curr_cache)
            X_curr = X_next
        
        Wl = self.params['W'+str(self.num_layers)]
        bl = self.params['b'+str(self.num_layers)]
        scores, curr_cache = affine_forward(X_curr, Wl, bl)
        cache.append(curr_cache)
        ...
        loss, dx = softmax_loss(scores, y)
        dout, grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)] = affine_backward(dx, cache[self.num_layers-1])
        for i in range(self.num_layers-2, -1, -1):
            ...
            if self.normalization != None:
                dout, grads['W'+str(i+1)], grads['b'+str(i+1)], grads['gamma'+str(i+1)], grads['beta'+str(i+1)] =                                                              affine_norm_relu_backward(dout, cache[i], self.normalization, self.use_dropout)
            else:
                dout, grads['W'+str(i+1)], grads['b'+str(i+1)], _, _ = affine_norm_relu_backward(dout, cache[i],                                                                                                     self.normalization, self.use_dropout)
